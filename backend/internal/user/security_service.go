...
func (s *SecurityService) Enable2FA(ctx context.Context, userID uuid.UUID, tenantID *uuid.UUID, token string) error {
	...
	// Update user record
	if err := s.db.WithContext(ctx).Model(&User{}).Where("id = ?", userID).Update("two_factor_enabled", true).Error; err != nil {
		return err
	}
...
func (s *SecurityService) ResetFailedLogins(ctx context.Context, userID uuid.UUID) error {
	...
	lockout, err := s.getOrCreateLockout(ctx, userID, user.TenantID)
	if err != nil {
		return err
	}
...
func (s *SecurityService) Disable2FA(ctx context.Context, userID uuid.UUID) error {
	...
	// Update user record
	if err := s.db.WithContext(ctx).Model(&User{}).Where("id = ?", userID).Update("two_factor_enabled", false).Error; err != nil {
		return err
	}

	// Log 2FA disabled
	return s.LogSecurityEvent(ctx, userID, user.TenantID, Event2FADisabled, "", "", nil)
}
...
func (s *SecurityService) VerifyBackupCode(ctx context.Context, userID uuid.UUID, code string) (bool, error) {
	...
	for i, backupCode := range twoFA.BackupCodes {
		if backupCode == code {
			...
			// Log backup code usage
			s.LogSecurityEvent(ctx, userID, user.TenantID, EventBackupCodeUsed, "", "", map[string]interface{}{
				"backup_code_used": true,
			})
			return true, nil
		}
